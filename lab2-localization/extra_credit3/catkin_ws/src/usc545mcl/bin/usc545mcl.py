...
class ParticleFilter(object):
    ...
    def GetClusteredPose(self):
        fullpos = [np.append(p.map_T_particle.translation, p.map_T_particle.rotation) for p in self.particles]
        ...
        id = labels == largest
        clustered_particles = [p for p, c in zip(self.particles, id) if c]
        ...
        avg_translation = sum_translation / len(clustered_particles)
        avg_rotation = sum_rotation / len(clustered_particles)
    ...
    def UpdateScan(self, scan_msg):
        ...
        avg_pose = self.GetClusteredPose()
