...
ODOMETRY_MODEL_VAR_1 = 0.3
ODOMETRY_MODEL_VAR_2 = 0.2
ODOMETRY_MODEL_VAR_3 = 0.2
ODOMETRY_MODEL_VAR_4 = 0.3
...
class Particle(object):
    ...
    def UpdateOdom(self, odom_msg):
        ...
        v_map = RotateBy(np.array([vx, vy], dtype=dtype), self.map_T_particle.rotation)
        vx, vy = v_map[0], v_map[1]

        d_rot1 = np.arctan2(vy, vx) - self.map_T_particle.rotation
        d_trans = np.sqrt(vx**2 + vy**2) * dt
        d_rot2 = wz * dt - d_rot1

        sigma_d_rot1 = ODOMETRY_MODEL_VAR_1 * np.abs(d_rot1) + ODOMETRY_MODEL_VAR_2 * d_trans
        sigma_d_trans = ODOMETRY_MODEL_VAR_3 * d_trans + ODOMETRY_MODEL_VAR_4 * (np.abs(d_rot1) + np.abs(d_rot2))
        sigma_d_rot2 = ODOMETRY_MODEL_VAR_1 * np.abs(d_rot2) + ODOMETRY_MODEL_VAR_2 * d_trans

        d_rot1_noisy = d_rot1 + np.random.normal(0, sigma_d_rot1)
        d_trans_noisy = d_trans + np.random.normal(0, sigma_d_trans)
        d_rot2_noisy = d_rot2 + np.random.normal(0, sigma_d_rot2)

        self.map_T_particle.translation[0] += d_trans_noisy * np.cos(self.map_T_particle.rotation + d_rot1_noisy)
        self.map_T_particle.translation[1] += d_trans_noisy * np.sin(self.map_T_particle.rotation + d_rot1_noisy)
        self.map_T_particle.rotation += d_rot1_noisy + d_rot2_noisy
