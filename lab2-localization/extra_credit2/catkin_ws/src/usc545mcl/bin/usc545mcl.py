...
MAX_Z = 20.1409549713
SIGMA_HIT = 10
STD_DEV_HIT = np.sqrt(SIGMA_HIT)
LAMBDA_SHORT = 10 # Rougly half of MAX_Z
Z_HIT = 0.75
# Since in our case we don't have any dynamic obstacles, 
# we should have low value for Z_SHORT
Z_SHORT = 0.1
# There's also less chance of the sensor missing an object 
# or having the ray being absorbed by black objects in our case
Z_MAX = 0.1
Z_RAND = 0.05
Z = np.array([Z_HIT, Z_SHORT, Z_MAX, Z_RAND], dtype=dtype)
...
class Particle(object):
    ...
    def UpdateOdom(self, odom_msg):
        ...
        vx_noisy = vx + np.random.normal(0, np.sqrt(LINEAR_MODEL_VAR_X))
        vy_noisy = vy + np.random.normal(0, np.sqrt(LINEAR_MODEL_VAR_Y))

        v_map = RotateBy(np.array([vx_noisy, vy_noisy], dtype=dtype), self.map_T_particle.rotation)

        self.map_T_particle.translation += v_map * dt

        wz = odom_msg.twist.twist.angular.z
        wz_noisy = wz + np.random.normal(0, np.sqrt(ANGULAR_MODEL_VAR))

        self.map_T_particle.rotation += wz_noisy * dt
    ...
    def UpdateScan(self, scan):
        ...
        # Non-vectorized
        # beam_likelihoods = []
        # for i, sim_range in enumerate(sim_ranges):
        #     # beam_likelihood = np.exp(-0.5 * np.square(scan.ranges[i] - sim_range) / SENSOR_MODEL_VAR) / np.sqrt(2 * np.pi * SENSOR_MODEL_VAR)
        #     a, b = (0 - sim_range)/STD_DEV_HIT, (MAX_Z - sim_range)/STD_DEV_HIT
        #     p_hit = truncnorm.pdf(scan.ranges[i], a, b, loc=sim_range, scale=STD_DEV_HIT)
        #     eta_short = 1 / (1 - np.exp(-LAMBDA_SHORT*sim_range) + 1e-6)
        #     p_short = 0
        #     if scan.ranges[i] <= sim_range:
        #         p_short = eta_short * LAMBDA_SHORT * np.exp(-LAMBDA_SHORT*scan.ranges[i])
        #     p_max = scan.ranges[i] == MAX_Z
        #     p_rand = 1 / MAX_Z
        #     P = np.array([p_hit, p_short, p_max, p_rand], dtype=dtype)
        #     beam_likelihood = np.matmul(Z, P)
        #     beam_likelihoods.append(beam_likelihood)

        # self.weight = np.product(beam_likelihoods)
        sim_ranges = np.expand_dims(sim_ranges, axis=1)
        if len(scan.ranges.shape) < 2:
            scan.ranges = np.expand_dims(scan.ranges, axis=1)
        p_hit = np.zeros_like(sim_ranges)
        for i, sim_range in enumerate(sim_ranges):
            a, b = (0 - sim_range) / STD_DEV_HIT, (MAX_Z - sim_range)/STD_DEV_HIT
            p_hit[i] = truncnorm.pdf(scan.ranges[i], a, b, loc=sim_range, scale=STD_DEV_HIT)
        eta_short = 1 / (1 - np.exp(-LAMBDA_SHORT*sim_ranges) + 1e-6)
        p_short = eta_short * LAMBDA_SHORT * np.exp(-LAMBDA_SHORT*scan.ranges) * (scan.ranges <= sim_ranges)
        p_max = scan.ranges == MAX_Z
        p_rand = np.ones_like(p_max) * 1 / MAX_Z
        P = np.hstack((p_hit, p_short, p_max, p_rand))
        beam_likelihoods = np.matmul(P,Z)
        self.weight = np.product(beam_likelihoods)
