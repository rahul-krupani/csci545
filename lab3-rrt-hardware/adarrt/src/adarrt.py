...
class AdaRRT():
    ...
    def _get_random_sample(self):
        ...
        """
        # FILL in your code here
        return np.random.uniform(low=self.joint_lower_limits, high=self.joint_upper_limits)
    ...
    def _get_nearest_neighbor(self, sample):
        ...
        # FILL in your code here
        neighbor = self.start
        min_distance = np.linalg.norm(neighbor.state - sample)
        new_neighbor = neighbor

        if neighbor.children:
            for node in neighbor.children:
                node, distance = self._dfs(node, sample)
    ...
    def _dfs(self, node, sample):
        neighbor = node
        min_distance = np.linalg.norm(node.state - sample)
        new_neighbor = node
    ...
    def _extend_sample(self, sample, neighbor):
        ...
        """
        # FILL in your code here
        new_state = neighbor.state + self.step_size * (sample - neighbor.state) / np.linalg.norm(sample - neighbor.state)
    ...
    def _check_for_completion(self, node):
        ...
        """
        # FILL in your code here
        if np.linalg.norm(node.state - self.goal.state) - self.goal_precision <= 0:
            return True
        return False
    ...
    def _trace_path_from_start(self, node=None):
        ...
        while not np.all(np.equal(node.state, self.start.state)):
            node = node.parent
    ...
    def _get_random_sample_near_goal(self):
        offset = np.random.uniform(-0.05, 0.05, size=self.goal.state.shape)
        return self.goal.state + offset
