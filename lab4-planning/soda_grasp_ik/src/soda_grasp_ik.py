...
def try_shortcut(start_pos, end_pos, ada, collision_constraint, sequence):
    ...
    for point in sequence:
        ...
        if not collision_constraint.is_satisfied(state_space, skeleton, interpolated):
            return False
    return True
...
def main(if_sim):
    ...
    if if_sim:
        ...
        raw_input(
            "Please move arm to home position with the joystick. "
    ...
    # x, y, z, rw, rx, ry, rz
    table_pose = [0.3, 0.0, -0.75, 0.707107, 0.0, 0.0, 0.707107]
    table = world.add_body_from_urdf(tableURDFUri, table_pose)

    # add collision constraints
    collision_free_constraint = ada.set_up_collision_detection(
        ada.get_arm_state_space(), ada.get_arm_skeleton(), [soda, bowl, table]
    )
    full_collision_constraint = ada.get_full_collision_constraint(
        ada.get_arm_state_space(), ada.get_arm_skeleton(), collision_free_constraint
    )
    ...
    # set up IK generator
    ik_sampleable = adapy.create_ik(arm_skeleton, arm_state_space, sodaTSR, hand_node)
    ...
    while delta_z < target_delta_z:
        del_x = np.array([0, 0, 0, -0.01, 0, 0])
        J = arm_skeleton.get_jacobian(hand.get_endeffector_body_node())
        q = arm_skeleton.get_positions()
        del_q = np.matmul(np.linalg.pinv(J), del_x)
        q = q - del_q
